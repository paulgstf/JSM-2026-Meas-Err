---
title: "Misclassification Activity for JSM 2026 Short Course"
author: "set below"
date: "set below"
output: 
  beamer_presentation:
    ...
    colortheme: "orchid"
header-includes:
  - \AtBeginDocument{\title[]{Misclassification Activity\\JSM 2026 Short Course\\(with solutions)}}
  - \AtBeginDocument{\author[]{Paul Gustafson}}
  - \AtBeginDocument{\date[]{August 2, 2026}}
---
...
```{r}
greenland <- matrix(c(45, 94, 257, 945),
  dimnames = list(c("LC+", "LC-"), c("Rsn+", "Rsn-")),
  nrow = 2, byrow = TRUE)
...
```{r}
logOR.hat <- sum(c(1,-1,-1,1)*log(as.vector(greenland)))

logOR.SE <- sqrt(sum(1/as.vector(greenland)))

exp(logOR.hat + c(0, -1.96, 1.96)*logOR.SE)
```

## Assuming nondifferential exposure misclassification with 80% sensitivity and 90% specificity
...
ft <- misclassification(greenland,
...
Check you can reproduce one of the **differential** classification adjustments given in the slides (i.e., one of the off-diagonal table entries on slide 174).
...
```{r, message=F}
ft <- probsens(greenland,
 type="exposure", reps=5000, 
 seca = list("triangular", c(0.80, 0.90, 0.85)),
 spca = list("triangular", c(0.80, 0.90, 0.85)))       
...
```{r, message=F}
ft.more <- probsens(4*greenland,
 type="exposure", reps=5000, 
 seca = list("triangular", c(0.80, 0.90, 0.85)),
 spca = list("triangular", c(0.80, 0.90, 0.85)))       

# OR inference
ft.more$adj_measures[4,]
```
